...
/**
 * MESSAGE_EDITED 처리 핸들러.
 * - messages 프로젝션 UPDATE (content / edited_at)
 * - events 권위 로그에 INSERT
 *
 * 검증: 동일 세션 소속 / 본인 작성 / 미삭제.
 * EventService 의 @Transactional 안에서 호출되므로 race 처리는 직접 하지 않는다.
 */
@Component
@RequiredArgsConstructor
public class MessageEditedHandler implements EventHandler {
    ...
    public EventType supports() {
        return EventType.MESSAGE_EDITED;
    }
    ...
    public Event handle(Session session,
        ...
        // 3) projection UPDATE — content / edited_at 갱신
        message.edit(content);

        // 4) events INSERT — messageId + 갱신된 content 저장
